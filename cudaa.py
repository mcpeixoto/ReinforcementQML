...
EPSILON_MIN = 0.01
EPSILON_DECAY = 0.99
TARGET_UPDATE_FREQ = 5 # Steps between target network training
ONLINE_TRAIN_FREQ = 1 # Steps between online network training
EPISODES = 500
# 10 episodes is a win TODO
...
class QNetwork(nn.Module):
    def __init__(self, n_qubits, output_dim, n_layers=5, datareuploading=True):
        ...
        if datareuploading:
            self.input_weights = torch.nn.Parameter(torch.randn(n_layers, n_qubits, requires_grad=True, device="cuda"))
        else:
            self.input_weights = torch.nn.Parameter(torch.randn(1, n_qubits, requires_grad=True, device="cuda"))
        self.θ = torch.nn.Parameter(torch.randn(n_layers, n_qubits, 2, requires_grad=True, device="cuda"))
        self.output_weights = torch.nn.Parameter(torch.randn(output_dim, requires_grad=True, device="cuda"))
...
def train(agent, env, replay_buffer, target_network, optimizer, episodes):
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            # Train agent
            if len(replay_buffer) > BATCH_SIZE and step % ONLINE_TRAIN_FREQ == 0:
...
def main():
    ...
    optimizer = optim.Adam([{'params': agent.output_weights, 'lr': 100*LEARNING_RATE},
                            ...
                            {'params': agent.input_weights, 'lr': LEARNING_RATE}])
